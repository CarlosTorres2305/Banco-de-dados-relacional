...
SELECT p.nome AS nome_da_peca, SUM(e.quantidade) AS soma_quantidades
FROM tbl_peca p
...
SELECT cidade, COUNT(*) AS quantidade_de_fornecedores
FROM tbl_fornecedor
GROUP BY cidade;


--4 listar quantidade de peças de cada cor
SELECT cor, COUNT(*) AS quantidade_de_pecas
FROM tbl_peca
GROUP BY cor;
