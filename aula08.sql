...
WHERE NOT EXISTS (
    SELECT 1
    FROM tbl_estoque
    WHERE tbl_estoque.cod_peca = tbl_peca.cod_peca
);
...
WHERE cod_peca IN (
    ...
    WHERE EXISTS (
        SELECT 1
        FROM tbl_fornecedor
        WHERE tbl_estoque.cod_fornecedor = tbl_fornecedor.cod_fornecedor
        AND tbl_fornecedor.cidade = 'LONDRES'
    )
) 
AND cod_peca NOT IN (
    SELECT cod_peca
    ...
    WHERE EXISTS (
        ...
        FROM tbl_fornecedor
        WHERE tbl_estoque.cod_fornecedor = tbl_fornecedor.cod_fornecedor
        AND tbl_fornecedor.cidade = 'PARIS'
    )
);
